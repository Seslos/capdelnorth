<section>
<h1 class="text-2xl font-bold"> METODOLOGIA/RECURSOS - EQUIPAMIENTO</h1>
<hr class="mt-2 mb-6">
            ...
            <div class="card-body bg-gray-100">
                @if ($audience->id == $item->id)
                    <form wire:submit.prevent="update">
                        <input wire:model="audience.name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50  "> 
                    @error('audience.name')
    ...
    <article class="card">
        <div class="card-body bg-gray-100" >
            <form wire:submit.prevent="store">
                <input wire:model="name" class="h-10 border mt-1 rounded px-4 w-full bg-gray-50" placeholder="Agregar la Metodologia/Recursos - Equipamiento del curso">
               @error('name')
